...
- **LLM-коррекция терминов** — glm-5.3-flash (z.ai, `reasoning_effort=low`): regex-препасс по встроенному словарю + один LLM-вызов на весь транскрипт (до 10000 слов; свыше — рекурсивное уполовинивание); без ключа честно деградирует до regex-результата
...
Ключ z.ai (GLM) в `~/.hermes/.env`:

```
GLM_API_KEY=...
# опционально: GLM_BASE_URL=https://api.z.ai/api/paas/v4
```
