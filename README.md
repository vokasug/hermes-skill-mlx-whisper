...
- **LLM-коррекция терминов** — deepseek-flash (DeepSeek, `reasoning_effort=low`): regex-препасс по встроенному словарю + один LLM-вызов на весь транскрипт (до 10000 слов; свыше — рекурсивное уполовинивание); без ключа честно деградирует до regex-результата
- **Субтитры как «второе мнение»** (`--subs file.srt`) — если к аудио есть субтитры (например, с YouTube), LLM сверяет с ними ослышки, а из субтитров авто-извлекаются канон-термины. Решения принимает модель по своим знаниям; принятие её вывода — два детерминированных гейта в коде: word-diff бюджет (никаких переписываний текста) и числовой гейт (числа всегда остаются от Whisper, попытки изменения откатываются и логируются)
...
Ключ DeepSeek в `~/.hermes/.env`:

```
DEEPSEEK_API_KEY=...
# опционально: DEEPSEEK_BASE_URL=https://api.deepseek.com
```
...
- `--subs <srt/vtt>` — субтитры того же контента как второе мнение для LLM-этапа + авто-экстракция терминов из них
...
├── tests/
│  └── test_llm_gates.py      # офлайн-тесты LLM-гейтов (без API)
└── scripts/
