...
- **LLM-коррекция терминов** — deepseek-flash (DeepSeek V4.1-Flash, `reasoning_effort=low`): regex-препасс по встроенному словарю + один LLM-вызов на весь транскрипт (≤8000 слов; иначе чанки ~1200 слов × 10 потоков); без ключа честно деградирует до regex-результата
