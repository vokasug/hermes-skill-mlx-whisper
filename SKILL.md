...
- Основной MD-файл: шапка метаданных (дата, источник, субтитры, длительность, модель,
  LLM-коррекция, время этапов) + транскрипт блоками `**mm:ss**`. MD быстрого пути
...
1. Скрипт напечатал `OK /Users/alexander/result-mlx-whisper/YYYY-MM-DD_<имя>.md`; файл существует и непустой (`read_file`): шапка с метаданными (дата, источник, субтитры, длительность, модель, LLM-коррекция, время) и транскрипт блоками `**mm:ss** текст` с пустой строкой после каждого блока.
