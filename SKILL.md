...
без LLM: сравнение |старт − target| текущей и следующей границы). Каждый блок — одна строка
...
- `--subs <srt/vtt>` — субтитры как ВТОРОЕ МНЕНИЕ для LLM-коррекции (не референс — см. гейты ниже).
  ...
  (строка «Субтитры»).
- **LLM-этап = один вызов на весь транскрипт** (≤10000 слов; свыше — рекурсивное уполовинивание,
  половины параллельно): консистентный регистр терминов по всему документу; при
  ошибке/невалидном выводе кусок делится пополам дальше, единичный сегмент — regex-only.
  Один большой вызов дешевле нарезки на куски: reasoning оплачивается за каждый вызов заново.
- **Архитектура «советчик + судья»**: промпт — просьба, которую модель периодически нарушает.
  Принятие вывода решает только детерминированный код, ДВА гейта: (а) word-diff бюджет —
  пословный difflib входа/вывода: замены любого размера + вставки/удаления ≤10 слов суммарно
  и ≤3 за один рывок, больше — весь вывод отклоняется (regex-only). Точное равенство числа
  ...
  слова подставляются на место правки), лог «ОТКЛОНЕНО: числа». Хорошие числовые конверсии тоже
  отклоняются — осознанный консерватизм.
...
- Быстрый MD без LLM и VAD: `python3 ~/.hermes/skills/media/mlx-whisper/scripts/transcribe_to_md.py <аудио>`
- Основной MD-файл: шапка = блок контент-метаданных (название, автор, дата публикации, ссылка —
  ...
  длительность, модель, LLM-коррекция, время этапов); далее транскрипт блоками `**mm:ss**`. MD быстрого пути
  (`transcribe_to_md.py`) дополнительно содержит таблицу сегментов.
...
- initial-prompt НЕ исправляет термины и даёт регрессы — только для стиля; орфографию терминов
  чинит LLM-этап по канон-списку.
...
0. Офлайн-тесты LLM-гейтов (без API, llm_call подменён): `~/.local/share/uv/tools/mlx-whisper/bin/python ~/.hermes/skills/media/mlx-whisper/tests/test_llm_gates.py` → `ALL TESTS OK`. Прогонять после любой правки correct_stage/verify_llm_out/числового гейта/redistribute/read_llm_config.

1. Скрипт напечатал `OK /Users/alexander/result-mlx-whisper/YYYY-MM-DD_<имя>.md`; файл существует и непустой (`read_file`): шапка с блоком контент-метаданных (Название/Автор/Дата публикации/Ссылка) и техническими метаданными (дата, источник, субтитры, длительность, модель, LLM-коррекция, время) и транскрипт блоками `**mm:ss** текст` с пустой строкой после каждого блока.
